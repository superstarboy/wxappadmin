...
class Express
{
     ...
     * 如果不存在则返回商品id，如果存在返回false
     * @return bool
     */
    public function getNotInRuleGoodsId()
    {
        if ($this->cityId) {
    ...
    {
        // 订单商品总金额
        $orderTotalPrice = array_sum(array_column($this->goodsList, 'total_price'));
        foreach ($this->goodsList as &$goods) {
    ...
    private function onCalcGoodsfreight(&$goods, $orderTotalPrice)
    {
        ...
        // 商品总重量
        $totalWeight = bcmul($goods['goods_sku']['goods_weight'], $goods['total_num'], 2);
        // 商品总数量or总重量
