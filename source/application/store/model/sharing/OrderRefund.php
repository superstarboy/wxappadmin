...
class OrderRefund extends OrderRefundModel
{
    ...
    public function audit($data)
    {
        ...
        try {
            ...
            // 发送模板消息
            (new Message)->refund(static::detail($this['order_refund_id']), $order['order_no'], 20);
            // 事务提交
            $this->commit();
            return true;
        } catch (\Exception $e) {
    ...
    public function receipt($data)
    {
        ...
        try {
            ...
            // 微信原路退回
            $WxPay = new WxPay(WxappModel::getWxappCache());
            $WxPay->refund($order['transaction_id'], $order['pay_price'], $data['refund_money']);
            // 订单详情
            $order = Order::detail($this['order_id']);
            // 发送模板消息
            (new Message)->refund(self::detail($this['order_refund_id']), $order['order_no'], 20);
            $this->commit();
            return true;
        } catch (\Exception $e) {
            $this->rollback();
    ...
    {
        return self::detail($this['order_refund_id']);
    }
