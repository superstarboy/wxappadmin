...
class Order extends OrderModel
{
    ...
    public function exportList($dataType, $query)
    {
        ...
        // 表格标题
        $tileArray = [
            '订单号', '订单类型', '商品名称', '单价', '数量', '付款金额', '运费金额', '下单时间',
            '买家', '买家留言', '收货人姓名', '联系电话', '收货人地址', '物流公司', '物流单号',
            '付款状态', '付款时间', '拼团状态', '发货状态', '发货时间', '收货状态', '收货时间', '订单状态',
            '微信支付交易号', '是否已评价'
        ];
        // 表格内容
        $dataArray = [];
        foreach ($list as $order) {
            ...
            foreach ($order['goods'] as $goods) {
                $dataArray[] = [
                    ...
                    '订单类型' => $this->filterValue($order['order_type']['text']),
                    ...
                    '拼团状态' => $this->filterValue($order['active']['status']['text']),
    ...
    private function setWhere($query)
    {
        ...
        if (isset($query['active_id'])) {
            $query['active_id'] > 0 && $this->where('order.active_id', '=', (int)$query['active_id']);
        }
    }
    ...
    {
        if (
            $this['pay_status']['value'] == 10
            || $this['delivery_status']['value'] == 20
            || (
                // 拼团订单验证拼单状态
                $this['order_type']['value'] == 20 && $this['active']['status']['value'] != 20
            )
        ) {
            $this->error = '该订单不合法';
            return false;
    ...
    private function deliveryMessage($order_id)
    {
        ...
        foreach ($orderIds as $orderId) {
            $Message->delivery(self::detail($orderId), 20);
        }
        return true;
    }
    ...
    public function refund()
    {
        ...
        $WxPay->refund($this['transaction_id'], $this['pay_price'], $this['pay_price']);
        // 更新订单状态
        return $this->save(['order_status' => 20, 'is_refund' => 1]) !== false;
    }
